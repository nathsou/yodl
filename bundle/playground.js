...
`}},d=Z,q=Object.keys(N).filter((e)=>/^examples\/[^/]+\.yodl$/.test(e)).sort(),w={write_source:{label:"Source",extension:"yodl",language:"yodl",description:"Resolved source, with imported declarations available to the compiler."},write_mono:{label:"Monomorphised",extension:"yodl",language:"yodl",description:"Generic modules specialised with concrete parameters."},write_typed:{label:"Typed",extension:"yodl",language:"yodl",description:"Expressions annotated with their resolved types and widths."},write_simplified:{label:"Simplified",extension:"yodl",language:"yodl",description:"Core representation with loops expanded and expressions simplified."},write_firrtl:{label:"FIRRTL",extension:"fir",language:"firrtl",description:"Hardware represented as ports, operations, registers, and connections."},write_low_firrtl:{label:"Low FIRRTL",extension:"fir",language:"firrtl",description:"FIRRTL after lowering passes, ready for downstream tools."},write_rtlil:{label:"RTLIL",extension:"il",language:"rtlil",description:"Hardware in the intermediate language used by Yosys."}},E="examples/Playground.yodl",z={mode:"tour",path:`tour/${d[0].file}`,stage:"write_firrtl"};function I(e){if(!e||typeof e!=="object")return!1;let t=e;return Object.hasOwn(w,t.stage)&&(t.mode==="tour"?d.some((o)=>`tour/${o.file}`===t.path):t.mode==="examples"&&(q.includes(t.path)||t.path===E))}function ee(e){let t=new TextEncoder().encode(JSON.stringify({...e,version:1})),o="";for(let n of t)o+=String.fromCharCode(n);return btoa(o).replaceAll("+","-").replaceAll("/","_").replace(/=+$/,"")}function te(e){if(!e.startsWith("#code="))return null;if(e.length>200000)throw Error("This shared program is too large to open.");try{let t=atob(e.slice(6).replaceAll("-","+").replaceAll("_","/")),o=JSON.parse(new TextDecoder().decode(Uint8Array.from(t,(n)=>n.charCodeAt(0))));if(o.version!==1||!I(o)||typeof o.source!=="string")throw Error();return o}catch{throw Error("This share link is invalid or uses an unsupported version.")}}function oe(e,t){let o=t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),n=e.match(new RegExp(`${o}:(\\d+)\\.(\\d+)-(\\d+)\\.(\\d+)`));if(!n)return null;return{startLineNumber:+n[1],startColumn:+n[2],endLineNumber:+n[3],endColumn:+n[4]}}var c;async function ne(){await new Promise((n,a)=>{let m=setTimeout(()=>a(Error("The code editor took too long to load. Check your connection and reload.")),15000),M=()=>{clearTimeout(m),n()},X=(he)=>{clearTimeout(m),a(he)},O=window.require;if(!O)return X(Error("The code editor could not load. Check your connection and reload."));O.config({paths:{vs:"https://cdnjs.cloudflare.com/ajax/libs/monaco-editor/0.56.0/min/vs"}}),O(["vs/editor/editor.main"],M,X)}),c=window.monaco,c.languages.register({id:"yodl"}),c.languages.setMonarchTokensProvider("yodl",{keywords:["declare","module","let","match","if","else","for","in","const","package","import","true","false"],typeKeywords:["uint","sint","bool","clock","type","Nat","Type"],wordOperators:["and","or","not","xor","nand","nor","xnor","shl","shr","andr","orr","xorr"],operators:["==","!=","<=",">=","<:",">:","+:","-:","..","..<","..=","+","-","*","/","%","=>","?",":",".","->","::"],symbols:/[=><!~?:&|+\-*/^%.]+/,tokenizer:{root:[[/\w+!/,"function"],[/\b[us]\d+\b/,"type"],[/[a-zA-Z_]\w*/,{cases:{"@keywords":"keyword","@typeKeywords":"type","@wordOperators":"operator","@default":"identifier"}}],[/"([^"\\]|\\.)*$/,"string.invalid"],[/"/,{token:"string.quote",bracket:"@open",next:"@string"}],[/'[^'\\]'/,"string"],[/'\\.'/,"string"],[/\/\/.*$/,"comment"],[/\b\d+'[bhod]?\w+\b/,"number"],[/\b\d+(_\d+)*\b/,"number"],[/@symbols/,{cases:{"@operators":"operator","@default":"delimiter"}}],[/[(){}\[\],;]/,"delimiter"],[/\s+/,"white"]],string:[[/[^\\"]+/,"string"],[/\\./,"string.escape"],[/"/,{token:"string.quote",bracket:"@close",next:"@pop"}]]}}),c.editor.defineTheme("yodl-light",{base:"vs",inherit:!0,rules:[{token:"function",foreground:"795E26",fontStyle:"bold"},{token:"operator",foreground:"0000FF"}],colors:{"editor.background":"#ffffff","editor.foreground":"#25362b","editorLineNumber.foreground":"#829087","editor.selectionBackground":"#d9ebdf","editor.lineHighlightBackground":"#f7f9f6"}}),c.editor.defineTheme("yodl-dark",{base:"vs-dark",inherit:!0,rules:[{token:"function",foreground:"DCDCAA",fontStyle:"bold"},{token:"operator",foreground:"569CD6"}],colors:{"editor.background":"#1d241f","editor.foreground":"#e1e9e2","editorLineNumber.foreground":"#73867a","editor.selectionBackground":"#344e3c","editor.lineHighlightBackground":"#242e27"}}),c.languages.setLanguageConfiguration("yodl",{comments:{lineComment:"//"},brackets:[["{","}"],["[","]"],["(",")"]],autoClosingPairs:[{open:"{",close:"}"},{open:"[",close:"]"},{open:"(",close:")"},{open:'"',close:'"'}]});for(let n of["firrtl","rtlil"])c.languages.register({id:n}),c.languages.setMonarchTokensProvider(n,{tokenizer:{root:[[n==="firrtl"?/;.*/:/#.*/,"comment"],[/"[^"\\]*(?:\\.[^"\\]*)*"/,"string"],[/\b(?:circuit|module|extmodule|input|output|wire|node|reg|regreset|mem|inst|of|when|else|connect|attribute|parameter|cell|end|process|sync|update|assign)\b/,"keyword"],[/\b(?:UInt|SInt|Clock|Reset|AsyncReset)\b/,"type"],[/\b(?:mux|add|sub|mul|and|or|xor|not|bits|cat|pad|eq|lt|gt)\b/,"function"],[/-?\b\d+(?:'[01xzm-]+)?\b/,"number"],[/[<>=:]+/,"operator"]]}});let e={automaticLayout:!0,minimap:{enabled:!1},scrollBeyondLastLine:!1,fontSize:14,lineHeight:23,fontFamily:'"SFMono-Regular", Consolas, "Liberation Mono", monospace',padding:{top:20,bottom:20},renderLineHighlight:"gutter",scrollbar:{useShadows:!1,verticalScrollbarSize:8,horizontalScrollbarSize:8},tabSize:4,insertSpaces:!0,fixedOverflowWidgets:!0};document.getElementById("input-panel").replaceChildren();let t=c.editor.create(document.getElementById("input-panel"),{...e,language:"yodl",ariaLabel:"Yodl source code"}),o=c.editor.create(document.getElementById("output-panel"),{...e,readOnly:!0,language:"firrtl",ariaLabel:"Compiled output"});return{input:t,output:o}}var i=(e)=>document.getElementById(e),s=(e)=>i(e),p=(e)=>i(e),ce="yodl-playground-v2:",j=!0;function y(e){try{return localStorage.getItem(ce+e)}catch{return j=!1,null}}function b(e,t){try{localStorage.setItem(ce+e,t)}catch{j=!1}}function S(e){let t=i("notice");t.textContent=e;let o=document.createElement("button");o.textContent="Dismiss",o.addEventListener("click",()=>{t.hidden=!0}),t.append(o),t.hidden=!1}function v(e,t="idle"){i("compile-status").textContent=e,i("compile-status").dataset.state=t}var de=matchMedia("(prefers-color-scheme: dark)");function B(){let e=p("theme-select").value,t=e==="dark"||e==="system"&&de.matches;if(document.documentElement.dataset.theme=t?"dark":"light",c)c.editor.setTheme(t?"yodl-dark":"yodl-light")}var ie=y("theme");p("theme-select").value=["light","dark","system"].includes(ie??"")?ie:"system";B();p("theme-select").addEventListener("change",()=>{b("theme",p("theme-select").value),B()});de.addEventListener("change",B);var l={...z};try{let e=JSON.parse(y("selection")??"null");if(I(e))l=e}catch{}var K=null,R="";try{let e=te(location.hash);if(e)l={mode:e.mode,path:e.path,stage:e.stage},K=e.source,R=`shared:${location.hash.slice(6)}`,S("Shared circuit opened. Your existing lesson and example drafts are kept separately.")}catch(e){S(e.message)}var r,T=!1,_=0,g="",H=-1,D,A,u,x=!1,ue=0,C=0,h=null,k=i("auto-compile");k.checked=y("auto")!=="false";var we=`// Start a new circuit here.
module Top(a: bool) -> (q: bool) {
    q = a
}
`,P=(e)=>N[e]??we,Y=()=>R||`draft:${l.path}`;function W(){return d.findIndex((e)=>`tour/${e.file}`===l.path)}function L(){if(b(Y(),r.input.getValue()),!R)b("selection",JSON.stringify(l));i("save-status").textContent=j?"Draft saved locally":"Draft not saved · storage unavailable",i("draft-badge").hidden=r.input.getValue()===P(l.path)}function pe(){let e=l.mode==="tour";s("tour-mode").setAttribute("aria-pressed",String(e)),s("examples-mode").setAttribute("aria-pressed",String(!e)),i("guide").hidden=!e,i("source-label").textContent=e?"Lesson":"Example";let t=p("source-selector");t.replaceChildren();let o=e?d.map((n,a)=>({value:`tour/${n.file}`,label:`${String(a+1).padStart(2,"0")} · ${n.title}`})):[{value:E,label:"New circuit"},...q.map((n)=>({value:n,label:n.split("/").at(-1)}))];for(let n of o)t.add(new Option(n.label,n.value));if(t.value=l.path,i("input-filename").textContent=l.path.split("/").at(-1),i("input-filename").title=l.path,e){let n=W(),a=d[n];i("lesson-position").textContent=`${String(n+1).padStart(2,"0")} / ${d.length}`,i("lesson-topic").textContent=a.topic,i("lesson-title").textContent=a.title,i("lesson-intro").textContent=a.intro,i("lesson-observe").textContent=a.observe,i("lesson-challenge").textContent=a.challenge,i("lesson-concepts").replaceChildren(...a.concepts.map((m)=>{let M=document.createElement("li");return M.textContent=m,M})),s("suggested-stage").textContent=`Show ${w[a.stage].label} →`,s("previous-lesson").disabled=n===0,s("next-lesson").disabled=!1,s("next-lesson").textContent=n===d.length-1?"Explore examples →":"Next lesson →"}p("pass-selector").value=l.stage,J()}function J(){let e=w[l.stage];i("stage-description").textContent=e.description,i("stage-command").textContent=l.stage,c.editor.setModelLanguage(r.output.getModel(),e.language)}function ge(){i("problems").hidden=!0,h=null,c.editor.setModelMarkers(r.input.getModel(),"yodl",[])}function G(){if(_++,C=++ue,clearTimeout(D),ge(),s("copy-output").disabled=!0,s("download-output").disabled=!0,v(g?"Source changed · output is out of date":"Ready to compile"),k.checked)D=setTimeout(f,500)}function Q(e){if(L(),R="",K=null,location.hash.startsWith("#code="))history.replaceState(null,"",location.pathname+location.search);l=e,T=!0,r.input.setValue(y(Y())??P(l.path)),T=!1,r.input.setScrollTop(0),r.output.setValue(""),g="",H=-1,pe(),L(),G()}function U(e){if(e===l.mode&&!R)return;let t=e==="tour"?z.path:E,o=y(`last:${e}`);if(I({mode:e,path:o,stage:"write_firrtl"}))t=o;Q({mode:e,path:t,stage:e==="tour"?d.find((n)=>`tour/${n.file}`===t).stage:"write_firrtl"})}function re(e){l.stage=e,p("pass-selector").value=e,r.output.setValue(""),g="",J(),L(),G()}function V(e){i("editors").dataset.view=e,s("source-tab").setAttribute("aria-pressed",String(e==="source")),s("output-tab").setAttribute("aria-pressed",String(e==="output")),r.input.layout(),r.output.layout()}function F(e){if(i("problems").hidden=!1,i("error-message").textContent=e,h=oe(e,l.path),s("jump-error").hidden=h===null,h){let t=r.input.getModel().validateRange(h);h=t,c.editor.setModelMarkers(r.input.getModel(),"yodl",[{...t,message:e,severity:c.MarkerSeverity.Error}])}v(g?"Compilation failed · showing previous output":"Compilation failed · check diagnostics","error")}function f(){if(clearTimeout(D),clearTimeout(A),x)u?.terminate(),u=void 0;x=!0;let e=++ue;C=e;let t=_;ge(),v("Compiling…","loading");try{if(!u)u=new Worker(new URL("./playground-worker.js",import.meta.url),{type:"module"});u.onmessage=(o)=>{if(o.data.id!==e)return;if(x=!1,clearTimeout(A),e!==C)return;let n=o.data;if(n.error!==void 0){F(n.error);return}g=n.output??"",H=t,r.output.setValue(g),J(),s("copy-output").disabled=!g,s("download-output").disabled=!g,v(`✓ Compiled · ${Math.round(n.duration)} ms`,"success")},u.onerror=()=>{if(x=!1,clearTimeout(A),u?.terminate(),u=void 0,e===C)F("The compiler worker could not run. Reload the page or try Compile again.")},u.postMessage({id:e,source:r.input.getValue(),path:l.path,stage:l.stage}),A=setTimeout(()=>{if(u?.terminate(),u=void 0,x=!1,e===C)F("Compilation exceeded 15 seconds. Try a smaller design or reduce compile-time loop bounds, then compile again.")},15000)}catch(o){x=!1,u?.terminate(),u=void 0,F(`Could not start the compiler: ${o.message}`)}}function ae(e,t){let o=URL.createObjectURL(new Blob([t],{type:"text/plain;charset=utf-8"})),n=document.createElement("a");n.href=o,n.download=e,n.click(),setTimeout(()=>URL.revokeObjectURL(o),1000)}async function se(e,t){try{await navigator.clipboard.writeText(e);let o=t.textContent;t.textContent="Copied",setTimeout(()=>{t.textContent=o},1800)}catch{if(S("Clipboard access is unavailable. Select the text and use your browser’s Copy command."),t.id==="copy-share")i("share-url").select();else r.output.focus(),r.output.setSelection(r.output.getModel().getFullModelRange())}}async function xe(){r=await ne(),B();for(let[t,o]of Object.entries(w))p("pass-selector").add(new Option(o.label,t));if(T=!0,r.input.setValue(y(Y())??K??P(l.path)),T=!1,pe(),matchMedia("(max-width: 820px)").matches)i("guide-details").open=!1;L();for(let t of["share-button","source-selector","compile-button","pass-selector","reset-button","download-source"])i(t).disabled=!1;let e=/Mac|iPhone|iPad/.test(navigator.platform);if(i("compile-shortcut").textContent=e?"⌘ ↵":"Ctrl ↵",r.input.addAction({id:"compile-yodl",label:"Compile Yodl",keybindings:[c.KeyMod.CtrlCmd|c.KeyCode.Enter],run:f}),r.output.addAction({id:"compile-yodl-output",label:"Compile Yodl",keybindings:[c.KeyMod.CtrlCmd|c.KeyCode.Enter],run:f}),document.addEventListener("keydown",(t)=>{if(!t.defaultPrevented&&(t.metaKey||t.ctrlKey)&&t.key==="Enter")t.preventDefault(),f()}),r.input.onDidChangeModelContent(()=>{if(T)return;L(),G()}),r.input.onDidChangeCursorPosition((t)=>{i("cursor-position").textContent=`Ln ${t.position.lineNumber}, Col ${t.position.column}`}),s("tour-mode").onclick=()=>U("tour"),s("examples-mode").onclick=()=>U("examples"),p("source-selector").onchange=()=>{let t=p("source-selector").value;b(`last:${l.mode}`,t),Q({mode:l.mode,path:t,stage:l.mode==="tour"?d.find((o)=>`tour/${o.file}`===t).stage:l.stage})},s("previous-lesson").onclick=()=>le(-1),s("next-lesson").onclick=()=>le(1),s("suggested-stage").onclick=()=>re(d[W()].stage),p("pass-selector").onchange=()=>re(p("pass-selector").value),s("compile-button").onclick=f,k.onchange=()=>{if(b("auto",String(k.checked)),clearTimeout(D),k.checked)f()},s("source-tab").onclick=()=>V("source"),s("output-tab").onclick=()=>V("output"),s("jump-error").onclick=()=>{if(!h)return;V("source"),r.input.setSelection(h),r.input.revealRangeInCenter(h),r.input.focus()},s("reset-button").onclick=()=>i("reset-dialog").showModal(),i("reset-dialog").addEventListener("close",()=>{if(i("reset-dialog").returnValue==="reset")r.input.setValue(P(l.path))}),s("download-source").onclick=()=>ae(l.path.split("/").at(-1),r.input.getValue()),s("download-output").onclick=()=>{if(H===_)ae(`${l.path.split("/").at(-1).replace(/\.yodl$/,"")}.${w[l.stage].extension}`,g)},s("copy-output").onclick=()=>{if(H===_)se(g,s("copy-output"))},s("share-button").onclick=()=>{let t=new URL(location.href);if(t.hash=`code=${ee({...l,source:r.input.getValue()})}`,t.href.length>32000){S("This circuit is too large for a reliable share link. Use Save to download the source instead.");return}i("share-url").value=t.href,i("share-dialog").showModal(),i("share-url").select()},s("copy-share").onclick=()=>void se(i("share-url").value,s("copy-share")),ke(),v("Ready to compile"),k.checked)f()}function le(e){let t=W()+e;if(t>=d.length){U("examples");return}let o=d[t];if(o)b("last:tour",`tour/${o.file}`),Q({mode:"tour",path:`tour/${o.file}`,stage:o.stage}),i("guide").scrollTop=0}function ke(){let e=i("resize-handle"),t=Number(y("split")??50);function o(a){t=Math.max(25,Math.min(75,Number.isFinite(a)?a:50)),i("editors").style.setProperty("--source-width",`${t}%`),e.setAttribute("aria-valuenow",String(Math.round(t))),r.input.layout(),r.output.layout()}o(t),e.onpointerdown=(a)=>{e.setPointerCapture(a.pointerId),e.classList.add("dragging"),a.preventDefault()},e.onpointermove=(a)=>{if(!e.hasPointerCapture(a.pointerId))return;let m=i("editors").getBoundingClientRect();o((a.clientX-m.left)/m.width*100)};let n=()=>{e.classList.remove("dragging"),b("split",String(t))};e.onlostpointercapture=n,e.onpointerup=(a)=>{if(e.hasPointerCapture(a.pointerId))e.releasePointerCapture(a.pointerId)},e.onkeydown=(a)=>{if(!["ArrowLeft","ArrowRight","Home","End"].includes(a.key))return;a.preventDefault(),o(a.key==="Home"?25:a.key==="End"?75:t+(a.key==="ArrowLeft"?-5:5)),n()}}xe().catch((e)=>{v("Could not load the editor","error"),i("input-panel").textContent="The editor could not load. Check your connection and reload the page.",S(`Playground startup failed: ${e.message??String(e)}`)});
