...
`,"examples/Testbench.yodl":`module XorGate(a: bool, b: bool) -> (out: bool) {
    out = a xor b
}
...
`}},h=Se,ne=Object.keys(R).filter((e)=>/^examples\/[^/]+\.yodl$/.test(e)).sort(),A="examples/Playground.yodl",se={mode:"tour",path:`tour/${h[0].file}`,stage:"write_firrtl"};function W(e){if(!e||typeof e!=="object")return!1;let t=e;return Object.hasOwn(C,t.stage)&&(t.mode==="tour"?h.some((i)=>`tour/${i.file}`===t.path):t.mode==="examples"&&(ne.includes(t.path)||t.path===A))}function Te(e){return xe({...e,version:e.entryPath?2:1})}function Me(e){if(!e.startsWith("#code="))return null;try{let t=ke(e.slice(6));if(![1,2].includes(t.version)||!W(t)||typeof t.source!=="string")throw Error();if(t.version===2&&(!ve(t.entryPath)||!Ce(t.files)||t.origin!==void 0&&!/^[a-zA-Z0-9_-]+\.html#[a-z0-9-]+$/.test(t.origin)))throw Error();if(t.version===1)return{version:1,mode:t.mode,path:t.path,stage:t.stage,source:t.source};return t}catch{throw Error("This share link is invalid, too large, or uses an unsupported version.")}}var o=(e)=>document.getElementById(e),r=(e)=>o(e),g=(e)=>o(e),De="yodl-playground-v2:",pe=!0;function B(e){try{return localStorage.getItem(De+e)}catch{return pe=!1,null}}function N(e,t){try{localStorage.setItem(De+e,t)}catch{pe=!1}}function V(e){let t=o("notice");t.textContent=e;let i=document.createElement("button");i.textContent="Dismiss",i.addEventListener("click",()=>{t.hidden=!0}),t.append(i),t.hidden=!1}function k(e,t="idle"){o("compile-status").textContent=e,o("compile-status").dataset.state=t}var Ge=Ae(g("theme-select"),(e)=>{if(y)y.editor.setTheme(e?"yodl-dark":"yodl-light")}),l={...se};try{let e=JSON.parse(B("selection")??"null");if(W(e))l=e}catch{}var Oe=new URLSearchParams(location.search),ae=h.find((e)=>e.id===Oe.get("lesson"));if(ae)l={mode:"tour",path:`tour/${ae.file}`,stage:ae.stage};else if(Oe.get("mode")==="examples")l={mode:"examples",path:A,stage:"write_firrtl"};var F={},E,j,q=null,U="";try{let e=Me(location.hash);if(e)l={mode:e.mode,path:e.path,stage:e.stage},q=e.source,F=e.files??{},E=e.entryPath,j=e.origin,U=`shared:${location.hash.slice(6)}`,V("Shared circuit opened. Your existing lesson and example drafts are kept separately.")}catch(e){V(e.message)}var c,z=!1,M,f="",D="",b=new Map,G=new Map,m=()=>E??l.path,L=()=>M.getValue();function Q(e){let t=e===m()?M:b.get(e);if(!t)return;if(f)G.set(f,c.input.saveViewState());f=e,c.input.setModel(t),c.input.updateOptions({readOnly:e!==m(),ariaLabel:`${e}${e===m()?", main source":", imported, read only"}`});let i=G.get(e);if(i)c.input.restoreViewState(i);me(),c.input.layout()}function me(){let e=f!==m(),t=b.size>0;o("source-files").hidden=!t,o("editors").dataset.imports=String(t),o("input-filename").textContent=f,o("input-filename").title=f,o("source-kind").textContent=e?"Imported · read only":"",o("source-kind").hidden=!e,o("draft-badge").hidden=e||L()===he(),r("reset-button").disabled=e,o("source-files").replaceChildren(...[m(),...b.keys()].map((i)=>{let n=document.createElement("button");return n.textContent=i.split("/").at(-1),n.title=i===m()?`${i} · compile and simulation target`:`${i} · imported, read only`,n.setAttribute("aria-pressed",String(i===f)),n.onclick=()=>Q(i),n}))}function Je(e){let t=Object.entries(e).filter(([n])=>n!==m()&&n.endsWith(".yodl")),i=new Set(t.map(([n])=>n));if(f!==m()&&!i.has(f))Q(m());for(let[n,a]of b)if(!i.has(n))a.dispose(),b.delete(n),G.delete(n);for(let[n,a]of t){let s=b.get(n);if(!s)b.set(n,y.editor.createModel(a,"yodl"));else if(s.getValue()!==a)s.setValue(a)}me()}function Xe(){f="",G.clear(),c.input.setModel(M);for(let e of b.values())e.dispose();b.clear(),Q(m())}var J=0,S="",X=-1,ze=new de,Ne=new de,re=0,Le;async function Ve(){let e=++re,t=await Ne.compile("imports",{source:L(),path:m(),stage:"write_source",files:{...R,...F}});if(e===re&&t?.sources)Je(t.sources)}function Ze(){++re,Ne.cancel("imports"),clearTimeout(Le),Le=setTimeout(Ve,150)}function Qe(e){return/\btest\s+(?:"|for\b)/.test(e)}function je(){r("run-tests").hidden=!Qe(L())}var T=new Fe,d="ready",qe=0,le=0,x=null,et=`// Start a new circuit here.
module Top(a: bool) -> (q: bool) {
    q = a
}
`,ee=(e)=>R[e]??et,he=()=>E&&q!==null?q:ee(l.path);function tt(e){let t=2166136261;for(let i=0;i<e.length;i++)t^=e.charCodeAt(i),t=Math.imul(t,16777619);return(t>>>0).toString(36)}var ge=()=>U||`draft:${l.path}:${tt(ee(l.path))}`;function Z(){return h.findIndex((e)=>`tour/${e.file}`===l.path)}function I(){if(N(ge(),L()),!U)N("selection",JSON.stringify(l));o("save-status").textContent=pe?"Draft saved locally":"Draft not saved · storage unavailable",o("draft-badge").hidden=f!==m()||L()===he()}function Be(){o("output-pane").dataset.view="output";let e=l.mode==="tour";o("site-section").textContent=e?"tour":"playground",r("tour-mode").setAttribute("aria-pressed",String(e)),r("examples-mode").setAttribute("aria-pressed",String(!e)),o("guide").hidden=!e,o("source-label").textContent=e?"Lesson":"Example";let t=g("source-selector");t.replaceChildren();let i=e?h.map((s,u)=>({value:`tour/${s.file}`,label:`${String(u+1).padStart(2,"0")} · ${s.title}`})):[{value:A,label:"New circuit"},...ne.map((s)=>({value:s,label:s.split("/").at(-1)}))];for(let s of i)t.add(new Option(s.label,s.value));t.value=l.path,me();let n=o("related-docs"),a=Object.entries(we).find(([,s])=>s.some((u)=>u.id===h[Z()]?.id));if(n.hidden=!a&&!j,n.href=`./book/${j??(a?a[0]+".html":"")}`,e){let s=Z(),u=h[s];o("lesson-position").textContent=`${String(s+1).padStart(2,"0")} / ${h.length}`,o("lesson-topic").textContent=u.topic,o("lesson-title").textContent=u.title,o("lesson-intro").textContent=u.intro,o("lesson-observe").textContent=u.observe,o("lesson-challenge").textContent=u.challenge,o("lesson-concepts").replaceChildren(...u.concepts.map((p)=>{let v=document.createElement("li");return v.textContent=p,v})),r("suggested-stage").textContent=`Show ${C[u.stage].label} →`,r("previous-lesson").disabled=s===0,r("next-lesson").disabled=!1,r("next-lesson").textContent=s===h.length-1?"Explore examples →":"Next lesson →"}g("pass-selector").value=l.stage,te()}function te(){let e=C[l.stage];o("stage-description").textContent=e.description,o("stage-description").title=e.description,g("pass-selector").title=e.description,o("stage-command").textContent=l.stage,y.editor.setModelLanguage(c.output.getModel(),e.language)}function Ue(){o("problems").hidden=!0,x=null;for(let e of[M,...b.values()])y.editor.setModelMarkers(e,"yodl",[])}function fe(){ze.cancel("playground"),T.stop(),d="ready",_(),J++,Ze(),le=++qe,Ue(),r("copy-output").disabled=!0,r("download-output").disabled=!0,k(S?"Source changed · output is out of date":"Ready to compile"),je()}function ye(e){if(I(),P([]),U="",q=null,F={},E=void 0,j=void 0,location.hash.startsWith("#code="))history.replaceState(null,"",location.pathname+location.search);l=e,Xe();for(let i of["simulation-top","simulation-clock","simulation-inputs"])o(i).value="";z=!0,c.input.setValue(B(ge())??ee(l.path)),z=!1,c.input.setScrollTop(0),c.output.setValue(""),S="",X=-1,Be();let t=new URL(location.href);t.searchParams.delete("lesson"),t.searchParams.delete("mode"),history.replaceState(null,"",t),I(),fe()}function ce(e){if(e===l.mode&&!U)return;let t=e==="tour"?se.path:A,i=B(`last:${e}`);if(W({mode:e,path:i,stage:"write_firrtl"}))t=i;ye({mode:e,path:t,stage:e==="tour"?h.find((n)=>`tour/${n.file}`===t).stage:"write_firrtl"})}function Ee(e){P([]),l.stage=e,g("pass-selector").value=e,c.output.setValue(""),S="",te(),o("output-pane").dataset.view="output",I(),fe()}function Y(e){o("editors").dataset.view=e,r("source-tab").setAttribute("aria-pressed",String(e==="source")),r("output-tab").setAttribute("aria-pressed",String(e==="output")),c.input.layout(),c.output.layout()}function it(e){if(o("output-pane").dataset.view="output",o("problems").hidden=!1,o("error-message").textContent=e,D=[m(),...b.keys()].find((t)=>K(e,t))??m(),x=K(e,D),r("jump-error").hidden=x===null,x){let t=D===m()?M:b.get(D),i=t.validateRange(x);x=i,y.editor.setModelMarkers(t,"yodl",[{...i,message:e,severity:y.MarkerSeverity.Error}])}k(S?"Compilation failed · showing previous output":"Compilation failed · check diagnostics","error")}async function O(){T.stop(),d="ready",_(),P([]);let e=++qe;le=e;let t=J;Ue(),k("Compiling…","loading");let i=await ze.compile("playground",{source:L(),path:E??l.path,stage:l.stage,files:{...R,...F}});if(!i||e!==le)return;if(i.error!==void 0){it(i.error);return}S=i.output??"",X=t,c.output.setValue(S),te(),o("output-pane").dataset.view="output",r("copy-output").disabled=!S,r("download-output").disabled=!S,k(`✓ Compiled · ${Math.round(i.duration)} ms`,"success")}async function ot(){if(l.stage!=="test")l.stage="test",g("pass-selector").value="test",te(),I();await O()}function Ke(e){let t={};for(let i of e.split(",")){let n=/^\s*([A-Za-z_$][\w$]*)(?::(\d+))?\s*=\s*(-?\d+)\s*$/.exec(i);if(!i.trim())continue;if(!n)throw Error(`Invalid input assignment: ${i}`);if(!Number.isSafeInteger(Number(n[3])))throw Error("Input exceeds the safe integer range.");t[n[1]]={width:Number(n[2]??32),value:Number(n[3])}}return t}function _(){let e=r("simulation-run"),t=r("simulation-stop");e.textContent=d==="running"||d==="stepping"?"Pause":d==="paused"?"Resume":"Run",e.disabled=d==="starting"||d==="halted",t.disabled=d==="ready"}var $e,ue;function P(e){let t=o("simulation-framebuffer"),i=e.at(-1);if(ue=i,o("simulation-zoom-control").hidden=!i,!i){t.hidden=!0;return}if(t.hidden=!1,t.width!==i.width||t.height!==i.height)t.width=i.width,t.height=i.height,$e=void 0;let n=t.parentElement?.clientWidth||640,a=g("simulation-zoom").value,s=a==="fit"?Math.min(n/i.width,480/i.height):Number(a);t.style.width=`${i.width*s}px`,t.style.height=`${i.height*s}px`;let u=t.getContext("2d");if(!u)return;let p=$e??=u.createImageData(i.width,i.height),v=Math.ceil(i.width/32);for(let w=0;w<i.width*i.height;w++){let We=Math.floor(w/i.width),ie=w%i.width,be=We*v+Math.floor(ie/32),oe=!(!i.valid||(i.packed?(i.valid[be]&1<<ie%32)!==0:i.valid[w]!==0))?16711935:i.packed?(i.packed[be]&1<<ie%32)!==0?i.onColor??16777215:i.offColor??0:i.rgb?.[w]??i.pixels?.[w]??0;p.data[w*4]=oe>>>16&255,p.data[w*4+1]=oe>>>8&255,p.data[w*4+2]=oe&255,p.data[w*4+3]=255}u.putImageData(p,0,0)}function nt(e,t,i,n){let a=[`cycles: ${i}`],s=e;for(let u of s.slice(0,100))a.push(`${u.name}: u${u.width} = ${u.known?u.value:"X"}`);if(s.length>100)a.push(`… ${s.length-100} more outputs`);if(t.length)a.push("",...t);o("simulation-output").textContent=a.join(`
`)}function st(e){let t=o("simulation-inputs-controls"),i=e.map((n)=>`${n.name}:${n.width}:${n.value}:${n.known}`).join("|");if(t.dataset.signature===i)return;t.dataset.signature=i,t.replaceChildren();for(let n of e){let a=document.createElement("label");a.textContent=n.name;let s=document.createElement("input");if(s.dataset.signal=n.name,s.dataset.width=String(n.width),n.width===1)s.type="checkbox",s.checked=n.value!=="0";else s.type="text",s.value=n.value,s.inputMode="numeric",s.title=`u${n.width}`;s.addEventListener("change",()=>{let u=[...t.querySelectorAll("input")].map((p)=>{let v=p.type==="checkbox"?Number(p.checked):p.value;return`${p.dataset.signal}:${p.dataset.width}=${v}`});o("simulation-inputs").value=u.join(", ");try{let p=Ke(u.join(", "));if(s.setCustomValidity(""),d!=="ready")T.setInputs(p)}catch(p){s.setCustomValidity(String(p)),s.reportValidity()}}),a.append(s),t.append(a)}t.hidden=e.length===0}function Re(e){if(e.type==="error"){d="error",o("simulation-output").textContent=e.error??"Simulation failed.",o("simulation-state").textContent="Error",_(),k("Simulation failed","error");return}if(d=e.type==="halted"?"halted":e.type==="stopped"?"ready":e.type==="stepping"?"stepping":e.type==="frame"||e.type==="resumed"?"running":"paused",e.frame)P([e.frame]);else if(e.metadata&&!e.metadata.display)P([]);if(e.outputs)nt(e.outputs,e.messages??[],e.totalCycles??0,e.frame?.signal);if(e.inputs)st(e.inputs);if(r("simulation-step-cycle").disabled=!e.clock||d==="halted",r("simulation-step-frame").hidden=!(e.frame&&e.clock),e.metadata){let u={"simulation-top":e.metadata.top,"simulation-clock":e.clock,"simulation-cycles-per-frame":e.playback?.cyclesPerFrame,"simulation-clock-hz":e.playback?.clockHz??"maximum","simulation-refresh-fps":e.playback?.refreshFps};for(let[p,v]of Object.entries(u))o(p).placeholder=String(v??"automatic");o("simulation-cycles-per-frame").disabled=!e.clock||Boolean(e.metadata.display?.stream),o("simulation-clock-hz").disabled=!e.clock,o("simulation-refresh-fps").disabled=!e.clock}let t=e.simulatedSeconds===void 0?"":` · ${e.simulatedSeconds.toFixed(3)} simulated s`,i=e.cyclesPerSecond===void 0?"":` · ${Math.round(e.cyclesPerSecond).toLocaleString()} cycles/s achieved`,n=e.status?.failed??!1,a=n?"Failed":d[0].toUpperCase()+d.slice(1),s=e.status?.exit_code===void 0?"":` · exit ${e.status.exit_code}`;o("simulation-state").textContent=`${a}${s} · ${(e.totalCycles??0).toLocaleString()} cycles${t}${i}`,_(),k(n?`Simulation failed${e.status?.first_failure?`: ${e.status.first_failure.message}`:""}`:d==="running"||d==="stepping"?"Simulating…":`Simulation ${d}`,n?"error":void 0)}async function H(e="run"){Y("output");let t=(s)=>{let u=Number(o(s).value);return Number.isFinite(u)&&u>0?u:void 0},i={clockHz:t("simulation-clock-hz"),refreshFps:t("simulation-refresh-fps"),cyclesPerFrame:t("simulation-cycles-per-frame")};if(d!=="ready"&&d!=="error"){if(e==="run")if(d==="running"||d==="stepping")T.pause();else T.resume(i);else T.command(e,i);return}let n=o("simulation-top").value.trim(),a=o("simulation-clock").value.trim();o("output-pane").dataset.view="simulation",o("simulation-state").textContent="Compiling simulation…",d="starting",_();try{T.start({source:L(),path:E??l.path,stage:"write_low_firrtl",files:{...R,...F},simulate:{action:e,...n?{top:n}:{},...a?{clock:a}:{},...Object.fromEntries(Object.entries(i).filter(([,s])=>s!==void 0)),inputs:Ke(o("simulation-inputs").value)}},Re)}catch(s){Re({id:0,type:"error",error:String(s)})}}function Ie(e,t){let i=URL.createObjectURL(new Blob([t],{type:"text/plain;charset=utf-8"})),n=document.createElement("a");n.href=i,n.download=e,n.click(),setTimeout(()=>URL.revokeObjectURL(i),1000)}async function _e(e,t){try{await navigator.clipboard.writeText(e);let i=t.textContent;t.textContent="Copied",setTimeout(()=>{t.textContent=i},1800)}catch{if(V("Clipboard access is unavailable. Select the text and use your browser’s Copy command."),t.id==="copy-share")o("share-url").select();else c.output.focus(),c.output.setSelection(c.output.getModel().getFullModelRange())}}async function at(){c=await He(),M=c.input.getModel(),f=m(),Ge();for(let[t,i]of Object.entries(C))g("pass-selector").add(new Option(i.label,t));if(z=!0,c.input.setValue(B(ge())??q??ee(l.path)),z=!1,Be(),matchMedia("(max-width: 820px)").matches)o("guide-details").open=!1;I();for(let t of["share-button","source-selector","compile-button","run-tests","simulate-button","simulation-reset","simulation-step-cycle","simulation-step-frame","simulation-stop","simulation-run","simulation-top","simulation-clock","simulation-cycles-per-frame","simulation-clock-hz","simulation-refresh-fps","simulation-inputs","pass-selector","reset-button","download-source"])o(t).disabled=!1;let e=/Mac|iPhone|iPad/.test(navigator.platform);o("compile-shortcut").textContent=e?"⌘ ↵":"Ctrl ↵",c.input.addAction({id:"compile-yodl",label:"Compile Yodl",keybindings:[y.KeyMod.CtrlCmd|y.KeyCode.Enter],run:O}),c.output.addAction({id:"compile-yodl-output",label:"Compile Yodl",keybindings:[y.KeyMod.CtrlCmd|y.KeyCode.Enter],run:O}),document.addEventListener("keydown",(t)=>{if(!t.defaultPrevented&&(t.metaKey||t.ctrlKey)&&t.key==="Enter")t.preventDefault(),O()}),c.input.onDidChangeModelContent(()=>{if(z||c.input.getModel()!==M)return;I(),fe()}),c.input.onDidChangeCursorPosition((t)=>{o("cursor-position").textContent=`Ln ${t.position.lineNumber}, Col ${t.position.column}`}),r("tour-mode").onclick=()=>ce("tour"),r("examples-mode").onclick=()=>ce("examples"),g("source-selector").onchange=()=>{let t=g("source-selector").value;N(`last:${l.mode}`,t),ye({mode:l.mode,path:t,stage:l.mode==="tour"?h.find((i)=>`tour/${i.file}`===t).stage:l.stage})},r("previous-lesson").onclick=()=>Pe(-1),r("next-lesson").onclick=()=>Pe(1),r("suggested-stage").onclick=()=>Ee(h[Z()].stage),g("pass-selector").onchange=()=>Ee(g("pass-selector").value),r("compile-button").onclick=O,r("run-tests").onclick=ot,r("simulate-button").onclick=()=>H("run"),r("simulation-run").onclick=()=>H("run"),r("simulation-reset").onclick=()=>H("reset"),r("simulation-step-cycle").onclick=()=>H("step_cycle"),r("simulation-step-frame").onclick=()=>H("step_frame"),g("simulation-zoom").onchange=()=>{if(ue)P([ue])},r("simulation-stop").onclick=()=>{T.stop(),d="ready",_(),o("simulation-state").textContent="Stopped",k("Simulation stopped")},r("source-tab").onclick=()=>Y("source"),r("output-tab").onclick=()=>Y("output"),r("jump-error").onclick=()=>{if(!x)return;Y("source"),Q(D),c.input.setSelection(x),c.input.revealRangeInCenter(x),c.input.focus()},r("reset-button").onclick=()=>o("reset-dialog").showModal(),o("reset-dialog").addEventListener("close",()=>{if(o("reset-dialog").returnValue==="reset")M.setValue(he())}),r("download-source").onclick=()=>Ie(f.split("/").at(-1),c.input.getValue()),r("download-output").onclick=()=>{if(X===J)Ie(`${l.path.split("/").at(-1).replace(/\.yodl$/,"")}.${C[l.stage].extension}`,S)},r("copy-output").onclick=()=>{if(X===J)_e(S,r("copy-output"))},r("share-button").onclick=()=>{let t=new URL(location.href);if(t.hash=`code=${Te({...l,source:L(),files:F,entryPath:E,origin:j})}`,t.href.length>32000){V("This circuit is too large for a reliable share link. Use Save to download the source instead.");return}o("share-url").value=t.href,o("share-dialog").showModal(),o("share-url").select()},r("copy-share").onclick=()=>void _e(o("share-url").value,r("copy-share")),rt(),k("Ready to compile"),Ve(),je()}function Pe(e){let t=Z()+e;if(t>=h.length){ce("examples");return}let i=h[t];if(i)N("last:tour",`tour/${i.file}`),ye({mode:"tour",path:`tour/${i.file}`,stage:i.stage}),o("guide").scrollTop=0}function rt(){let e=o("resize-handle"),t=Number(B("split")??50);function i(a){t=Math.max(25,Math.min(75,Number.isFinite(a)?a:50)),o("editors").style.setProperty("--source-width",`${t}%`),e.setAttribute("aria-valuenow",String(Math.round(t))),c.input.layout(),c.output.layout()}i(t),e.onpointerdown=(a)=>{e.setPointerCapture(a.pointerId),e.classList.add("dragging"),a.preventDefault()},e.onpointermove=(a)=>{if(!e.hasPointerCapture(a.pointerId))return;let s=o("editors").getBoundingClientRect();i((a.clientX-s.left)/s.width*100)};let n=()=>{e.classList.remove("dragging"),N("split",String(t))};e.onlostpointercapture=n,e.onpointerup=(a)=>{if(e.hasPointerCapture(a.pointerId))e.releasePointerCapture(a.pointerId)},e.onkeydown=(a)=>{if(!["ArrowLeft","ArrowRight","Home","End"].includes(a.key))return;a.preventDefault(),i(a.key==="Home"?25:a.key==="End"?75:t+(a.key==="ArrowLeft"?-5:5)),n()}}at().catch((e)=>{k("Could not load the editor","error"),o("input-panel").textContent="The editor could not load. Check your connection and reload the page.",V(`Playground startup failed: ${e.message??String(e)}`)});
